// export const form = document.querySelector('.add-form');
export const formBlock = document.querySelector('.form');
export const listComments = document.querySelector('.comments');
export const loading = document.querySelector('.load');
// export const inputComment = form.querySelector('.add-form-text');
// export const inputName = form.querySelector('.add-form-name');
// export const formButton = form.querySelector('.add-form-button');
export const autorizationButton = document.querySelector('.autorization-button');


